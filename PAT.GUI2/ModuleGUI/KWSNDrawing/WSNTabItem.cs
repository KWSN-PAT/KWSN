    ...
    class PNModelHelper
    {
        ...
        public bool generateXML(XmlDocument docRes)
        {
            ...
            do
            {
                ...
                {
                    data = sensor.GeneratePNXml(docRes, sensor.locateX - minX, sensor.locateY - minY); //(docRes, xStartPos, 0);
                    if (data == null)
                    {
                    ...
                    foreach (WSNSensorChannel channel in _channels)
                    {
                        ...
                        data = channel.GeneratePNXml(docRes, xStartPos - minX, yStartPos - minY);
                        if (data == null)
                        {
                            Log.d(TAG, "Failed to generate the sensor PN xml nodes");
                            blError = true;
